...
RULES = [
    ...
    (r"close.{0,10}ticket|close.{0,10}claim|close.{0,10}irp|closing.*ticket|can.t close|cannot close|can not close|ticket.*not clos|tickets need closed|close repair|close.*duplicate|mark.*ticket.*(close|reject)", "ticket_status", 0.85, "human_review"),
    (r"reopen|re-open",                                  "ticket_status",    0.85, "human_review"),
    (r"balance and close|force close",                   "ticket_status",    0.85, "human_review"),
    (r"cancel claim|reject claim|decline claim|claim.{0,15}(cancel|reject|decline)", "asurion_claim", 0.85, "human_review"),
    (r"google.*(claim|in.warranty|in-warranty|\biw\b|not authoris|warranty)", "google_claim", 0.85, "human_review"),
    (r"purchase order|reopen.*po|po.*reopen|receiving.*\bpo\b|internal transfer.*\bpo\b", "parts_order", 0.85, "human_review"),
    (r"invoice|remittance",                               "invoice_payment",  0.80, "human_review"),
    (r"\bcatalog\b",                                      "catalog_issue",    0.80, "human_review"),
    (r"lenovo.*(edi|depot|inventory|adjustment|failed)",  "lenovo_edi",       0.80, "human_review"),
