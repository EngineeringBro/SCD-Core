...
# Extraction request label — posted to GitHub Issues to trigger the local agent
EXTRACTION_LABEL = "scd-extraction-needed"
...
class OrphanedTransactionModule(Module):
    ...
    def run(self, ticket: dict, jira) -> ResolutionSuggestion:
        ...
        rq_ticket = _extract_rq_ticket(subject, fields)
        transaction_id = _extract_transaction_id(fields)
        transaction_date = _extract_transaction_date(fields)
        client_url = _extract_client_url(fields)
        client_name, client_source = _resolve_client(fields, client_url)
        base_url = CLIENT_TABLE.get(client_name, client_url or "unknown")
        known_client = client_name in CLIENT_TABLE

        # Check location cache — if the location is already known, pass it along
        # so the extraction agent can skip Steps 3+4 entirely
        location_name = _extract_location_name(fields)
        cache_hit = self._location_cache.get(location_name) if location_name else None

        diagnosis = (
            f"Orphaned transaction on {client_name} — "
            f"payment processed through Global Payments Terminal but not linked to "
            f"RQ ticket {rq_ticket or '(extract from ticket)'}. "
            f"Dispatching to local extraction agent for browser-based data extraction."
        )

        # Structured extraction request — parsed by the VS Code extraction agent
        extraction_request = {
            "scd_ticket": ticket_id,
            "rq_ticket": rq_ticket,
        ...
        return ResolutionSuggestion(
            ...
            evidence=[
                ...
                {"source": "client_source", "value": client_source},
                {"source": "rq_ticket_extracted", "value": rq_ticket or "not found"},
                {"source": "transaction_id_extracted", "value": transaction_id or "not found — agent will locate"},
                {"source": "transaction_date", "value": transaction_date or "not found — agent will locate"},
                {"source": "location_name", "value": location_name or "not found — agent will locate"},
                {"source": "location_cache_hit", "value": "yes" if cache_hit else "no"},
            ...
            module_notes=(
                "v1.1 dispatcher: identifies ticket + client + dispatches to local "
                "VS Code extraction agent via GitHub issue. Agent runs browser calls "
                "against authenticated Chrome session, posts filled SQL back to issue."
            ),
        )
...
def _extract_transaction_id(fields: dict) -> str | None:
    """Extract numeric transaction ID from description or comments."""
    all_text = _adf_to_text(fields.get("description") or {})
    for comment in (fields.get("comment") or {}).get("comments", []):
        all_text += " " + _adf_to_text(comment.get("body") or {})
    match = re.search(r"[Tt]ransaction\s*(?:ID|#)[^\d]*(\d{8,})", all_text)
    if match:
        return match.group(1)
    # bare 8+ digit number that could be a transaction ID
    match = re.search(r"\b(\d{8,})\b", all_text)
    return match.group(1) if match else None


def _extract_transaction_date(fields: dict) -> str | None:
    """Extract transaction date (MM/DD/YYYY) from description or comments."""
    all_text = _adf_to_text(fields.get("description") or {})
    for comment in (fields.get("comment") or {}).get("comments", []):
        all_text += " " + _adf_to_text(comment.get("body") or {})
    # ISO format first (2026-04-18)
    m = re.search(r"\b(\d{4}-\d{2}-\d{2})\b", all_text)
    if m:
...
def _extract_location_name(fields: dict) -> str | None:
    """Extract store/location name from description or comments."""
    all_text = _adf_to_text(fields.get("description") or {})
    for comment in (fields.get("comment") or {}).get("comments", []):
        all_text += " " + _adf_to_text(comment.get("body") or {})
    # "Mobile Klinik StoreName (NNN)" pattern
    m = re.search(r"Mobile\s+Klinik\s+([\w\s\-']+?\s*\(\d{3,}\))", all_text)
    if m:
        return f"Mobile Klinik {m.group(1).strip()}"
    # "Mobile Klinik StoreName" without store number
    m = re.search(r"Mobile\s+Klinik\s+([A-Z][A-Za-z\s\-']{3,30}?)(?:\s*[,\n(]|$)", all_text)
    return f"Mobile Klinik {m.group(1).strip()}" if m else None
